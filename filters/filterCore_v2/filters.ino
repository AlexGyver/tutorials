...
// медиана на N значений со своим буфером
float findMedianN(float newVal) {
  ...
  float ans = 0;
  if (NUM_READ % 2 == 0) {             // если кол-во элементов в массиве четное (NUM_READ - последний индекс массива, отсчет - с нуля)
    ans = buf[(int) (NUM_READ / 2)]; // то берем центральное
  } else {                              // иначе
